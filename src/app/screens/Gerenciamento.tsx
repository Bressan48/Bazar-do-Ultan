//Gerenciamento.tsx
import { Text, View, ImageBackground, Image, StyleSheet, TouchableOpacity, Alert } from "react-native";
...
export default function Gerenciamento() {
    ...
    try {
        //pega a primeira página para descobrir quantas existem
        const res = await fetch("https://treinamentoapi.codejr.com.br/api/bressan/products?page=1&per_page=10");
        const data = await res.json();
        let allProducts = [...data.products];//começa com a primeira página
        const totalPages = data.last_page;
        //se houver mais páginas, busca o resto
        if (totalPages > 1) {
        const requests = [];
